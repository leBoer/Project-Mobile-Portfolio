...
var adjectives = ["dark", "color", "whimsical", "shiny", "noisy", "apocalyptic",
...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    // Moved the variables outside the for-loop so that we don't have to calculate them repeatedly
    // Also changed from querySelectorAll to querySelector
    var dx = determineDx(document.querySelector(
        ".randomPizzaContainer"), size);
    var newwidth = (document.querySelector(".randomPizzaContainer")
            .offsetWidth + dx) + 'px';
    for (var i = 0; i < document.querySelectorAll(
        ".randomPizzaContainer").length; i++) {
      document.querySelectorAll(".randomPizzaContainer")[i].style
          .width = newwidth;
    }
  }
  ...
  console.log("Time to resize pizzas: " + timeToResize[timeToResize.length -
      1].duration + "ms");
};
...
function logAverageFrame(times) { // times is the array of User Timing measurements from updatePositions()
  ...
  console.log("Average scripting time to generate last 10 frames: " + sum /
      10 + "ms");
}
...
function updatePositions() {
  ...
  // Changed to two forEach functions to avoid FSL. This batches the updates.
  // Does create two instances of Forced Reflow, but does not significantly impact performance.
  var items = document.querySelectorAll('.mover');
  var phase = [];
  items.forEach(function(elem, index) {
    phase[index] = Math.sin((document.body.scrollTop / 1250) +
        (index % 5));
  });
  items.forEach(function(elem, index) {
    items[index].style.left = items[index].basicLeft + 100 *
        phase[index] + 'px';
  });
...
// Have reduced the number of animated pizzas from 200 to 30, which is enough to cover the display.
// Also reduced the file size of the pizza by 70%
